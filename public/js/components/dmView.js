...
function onDMMessageCreated(message) {
    ...
    if (dmState.currentDM?.id === message.dm_id) {
        ...
        // Track unread for DMs not currently open
        dmState.unread[message.dm_id] = (dmState.unread[message.dm_id] || 0) + 1;
        updateDMHomeBadge();
    }
    renderDMConversationList();

    // Browser notification when tab is not focused and DM is not currently open
    if (document.visibilityState !== 'visible' &&
        dmState.currentDM?.id !== message.dm_id &&
        state.currentUser && message.sender_id !== state.currentUser.id) {
        if ('Notification' in window && Notification.permission === 'granted') {
            const n = new Notification(`${message.username} sent you a message`, {
            ...
            n.onclick = () => {
                window.focus();
                if (!isInDMMode()) {
                    showDMHome().then(() => selectDMConversation(message.dm_id));
                } else {
                    selectDMConversation(message.dm_id);
                }
                n.close();
            };
        }
